...
export function SpinWheelScreen({ restaurants, onSelect, onBack }: SpinWheelScreenProps) {
  ...
  return (
    <div className="flex-1 w-full flex flex-col bg-white h-full relative overflow-hidden">
      {/* Header */}
      <div className="pt-12 px-6 pb-6 bg-[#F6F1EA] sticky top-0 z-10 border-b border-[#E7E5E4] shadow-sm shrink-0">
        <div className="relative flex items-center justify-between">
          <button 
            onClick={onBack} 
            className="w-10 h-10 flex items-center justify-center bg-white rounded-xl text-[#4B2E2A] border border-[#E7E5E4] hover:bg-[#E7E5E4] transition-all active:scale-90"
          >
            <ChevronLeft className="w-6 h-6" />
          </button>
          
          <div className="flex flex-col items-center">
            <h2 className="text-2xl font-black italic tracking-tighter text-[#4B2E2A]">Lucky Spin</h2>
          </div>
        ...
        <AnimatePresence mode="wait">
          {!showConfig ? (
            ...
            >
              ...
              {/* Count Selector */}
              <div className="flex bg-white p-1 rounded-2xl border border-[#E7E5E4] mb-12 shadow-sm">
                {[4, 6, 8, 12].map((count) => (
                  <button
                    key={count}
              ...
              <div className="relative group">
                ...
                <div className="absolute -inset-6 rounded-full bg-[#4B2E2A] shadow-[0_0_50px_rgba(255,97,29,0.2)] flex items-center justify-center">
              ...
              {winner && !isSpinning && (
                ...
                >
                  <div className="bg-white p-6 rounded-[2.3rem] border border-white/10">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <div className="flex items-center gap-1.5 bg-[#F6F1EA] px-3 py-1 rounded-full">
                        <Star className="w-3 h-3 fill-[#FFB80E] text-[#FFB80E]" />
                        <span className="text-xs font-bold text-[#4B2E2A]">{winner.rating}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-5 items-center">
                      <div className="relative shrink-0">
                        ...
                        <div className="absolute -bottom-2 -right-2 bg-white p-1 rounded-xl shadow-md border border-[#E7E5E4]">
                          <div className="bg-[#4B2E2A] text-white text-[8px] font-black px-2 py-1 rounded-lg">PRO</div>
                        </div>
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xl font-black text-[#4B2E2A] leading-tight mb-2 truncate">{winner.name}</h3>
                        <div className="flex items-center text-[#78716C] mb-4">
                          <MapPin className="w-3.5 h-3.5 mr-1.5 text-[#A8A29E]" />
                          <span className="text-xs font-bold">{winner.distance} dari lokasimu</span>
                        </div>
                        <button
            ...
            >
              <div className="mb-6">
                <h3 className="text-lg font-bold text-[#4B2E2A] mb-1">Kelola Pilihan</h3>
                <p className="text-xs text-[#78716C]">Pilih hingga 12 makanan untuk dimasukkan ke roda.</p>
              </div>

              <div className="bg-white rounded-2xl border border-[#E7E5E4] p-3 flex items-center gap-3 mb-6">
                ...
                {filteredRestaurants.map(r => (
                  ...
                  >
                    ...
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 transition-colors ${
                      selectedIds.includes(r.id) ? 'bg-[#FF611D] border-[#FF611D] text-white' : 'border-[#E7E5E4]'
                    }`}>
